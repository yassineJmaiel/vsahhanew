...
@section('content')
    ...
    <style>
        ...
        .stat-card.stat-blue:hover .stat-border,
        .stat-card.active-blue .stat-border,
        .stat-card.stat-green:hover .stat-border,
        .stat-card.active-green .stat-border,
        .stat-card.stat-yellow:hover .stat-border,
        .stat-card.active-yellow .stat-border,
        .stat-card.stat-red:hover .stat-border,
        .stat-card.active-red .stat-border {
            background: #FFFFFF !important;
            opacity: 1 !important;
        }

        .stat-card.stat-blue:hover .stat-value,
        .stat-card.stat-blue:hover .stat-label,
        .stat-card.active-blue .stat-value,
        .stat-card.active-blue .stat-label,
        .stat-card.stat-green:hover .stat-value,
        .stat-card.stat-green:hover .stat-label,
        .stat-card.active-green .stat-value,
        .stat-card.active-green .stat-label,
        .stat-card.stat-yellow:hover .stat-value,
        .stat-card.stat-yellow:hover .stat-label,
        .stat-card.active-yellow .stat-value,
        .stat-card.active-yellow .stat-label,
        .stat-card.stat-red:hover .stat-value,
        .stat-card.stat-red:hover .stat-label,
        .stat-card.active-red .stat-value,
        .stat-card.active-red .stat-label {
            color: #FFFFFF !important;
        }

        .stat-card.stat-blue:hover .stat-icon,
        .stat-card.active-blue .stat-icon,
        .stat-card.stat-green:hover .stat-icon,
        .stat-card.active-green .stat-icon,
        .stat-card.stat-yellow:hover .stat-icon,
        .stat-card.active-yellow .stat-icon,
        .stat-card.stat-red:hover .stat-icon,
        .stat-card.active-red .stat-icon {
            background: #FFFFFF !important;
        }

        .stat-card.stat-blue:hover .stat-icon svg path,
        .stat-card.active-blue .stat-icon svg path {
            stroke: #0B4F8A !important;
        }

        .stat-card.stat-green:hover .stat-icon svg path,
        .stat-card.active-green .stat-icon svg path {
            stroke: #10B981 !important;
        }

        .stat-card.stat-yellow:hover .stat-icon svg path,
        .stat-card.active-yellow .stat-icon svg path {
            stroke: #F59E0B !important;
        }

        .stat-card.stat-red:hover .stat-icon svg path,
        .stat-card.active-red .stat-icon svg path {
        ...
        .stat-mini-card.animated {
            animation: statCardFadeUp 0.45s cubic-bezier(.22,.68,0,1.2) forwards;
        }
        .stat-mini-bar {
            width: 0%;
        }
    ...
    <div class="container-fluid py-4">
        ...
        <div id="main-content">
            ...
            <div
                class="page-header-bar d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="page-header-subtitle mb-0">Manage relationships with insurance providers, brokers, and
                ...
                <div class="d-flex flex-wrap align-items-center gap-2">
                    ...
                    <button class="btn btn-primary d-inline-flex align-items-center gap-2 fw-semibold px-3 py-2"
                        style="background-color: #0B4F8A; border-color: #0B4F8A;"
                        onclick="window.location.href='{{ route('add-partner') }}'">
                        <i class="bi bi-plus-lg"></i>
                        <span>Add Partner</span>
                    </button>
                    <button
                        class="btn btn-outline-secondary d-inline-flex align-items-center gap-2 fw-semibold px-3 py-2 bg-white"
                        onclick="window.location.reload()">
                        <i class="bi bi-arrow-clockwise"></i>
                        <span>Refresh</span>
                    </button>
                    <button
                        class="btn btn-outline-secondary d-inline-flex align-items-center justify-content-center bg-white"
                            ...
                            <table id="partnerDataTable" class="table partner-table mb-0 align-middle w-100">
                                ...
                                <tbody>
                                    ...
                                    @foreach($partners as $partner)
                                        <tr class="partner-row" data-status="{{ $partner['status'] }}" style="cursor: pointer;"
                                            onclick="openPartnerDetails({{ json_encode($partner) }})">
                                            ...
                                            <td style="vertical-align: middle;">
                                                <div class="d-flex flex-column gap-2">
                                                    ...
                                                    <a href="#"
                                                        onclick="displayEdit({{ json_encode($partner) }}); event.stopPropagation();"
                                                        class="edit-partner-btn d-inline-flex align-items-center gap-1"
                                                        style="color: #0B4F8A; font-size: 13px; font-weight: 500; text-decoration: none;">
                                                        Edit Partner
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                                            viewBox="0 0 14 14" fill="none">
                                                            <path
                                                                d="M8.75027 2.91623L11.0838 5.24952M12.3521 3.97343C12.6605 3.6651 12.8338 3.24689 12.8339 2.81079C12.8339 2.37468 12.6607 1.95642 12.3523 1.64802C12.044 1.33961 11.6257 1.16631 11.1896 1.16626C10.7534 1.16621 10.3351 1.33939 10.0267 1.64772L2.24082 9.43451C2.10537 9.56955 2.0052 9.73581 1.94912 9.91866L1.17847 12.4573C1.1634 12.5077 1.16226 12.5613 1.17518 12.6124C1.1881 12.6634 1.2146 12.71 1.25186 12.7472C1.28912 12.7844 1.33577 12.8108 1.38683 12.8237C1.4379 12.8365 1.49149 12.8353 1.54192 12.8201L4.0814 12.0501C4.2641 11.9946 4.43036 11.895 4.56561 11.7602L12.3521 3.97343Z"
                                                                stroke="#0B4F8A" stroke-width="2" stroke-linecap="round" />
                                                        </svg>
                                                    </a>
                                                </div>
                    ...
                    <div id="partner-details-view" style="display: none;">
                        ...
                        <div class="card shadow-sm border-0 rounded-4 overflow-hidden mb-4">
                            ...
                            <div class="d-flex justify-content-between align-items-center"
                                style="height: 56px; padding: 0 20px; border-radius: 12px 12px 0 0; background: #0B4F8A;">
                                <div class="d-flex align-items-center" style="gap: 10px;">
                                    <div class="d-flex align-items-center justify-content-center"
                                        style="height: 28px; padding: 5px 3px 5px 7px; border-radius: 8px; background: rgba(255, 255, 255, 0.12); gap: 4px;">
                                    ...
                                    <h5 class="mb-0"
                                        style="color: #FFF; font-family: 'Inter', sans-serif; font-size: 16px; font-weight: 700;">
                                        Partner Details</h5>
                                </div>
                                <button type="button" class="btn text-white p-0 border-0 d-flex align-items-center"
                                    onclick="showPartnerList(event)">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20"
                                        fill="none" style="flex-shrink:0;">
                                        <g opacity="0.9" clip-path="url(#clip0_1007_538)">
                                            <path
                                                d="M12.5002 7.49982L7.49982 12.5002M7.49982 7.49982L12.5002 12.5002M18.334 10C18.334 14.6028 14.6028 18.334 10 18.334C5.39727 18.334 1.66602 14.6028 1.66602 10C1.66602 5.39727 5.39727 1.66602 10 1.66602C14.6028 1.66602 18.334 5.39727 18.334 10Z"
                            ...
                            <div class="card-body p-4 bg-white">
                                ...
                                <div class="rounded-4 bg-white mb-4"
                                    style="border: 1px solid #E2E8F0; box-shadow: 0 6px 16px -8px rgba(0, 0, 0, 0.04); padding: 16px;">
                                    <div
                                        class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                                        <!-- Left: Logo & Info -->
                                        <div class="d-flex align-items-center gap-3">
                                            <div id="detail_logo_box"
                                                class="d-flex flex-column justify-content-center align-items-center text-white fw-bold flex-shrink-0"
                                                style="width: 56px; height: 56px; border-radius: 14px; background: linear-gradient(90deg, #015AA7 0%, #1C4E7A 100%); font-family: 'Inter', sans-serif; font-size: 24px;">
                                                AP
                                            </div>
                                            <div class="d-flex flex-column" style="gap: 7px;">
                                                <h4 id="detail_partner_name" class="mb-0"
                                                    style="font-family: 'Inter', sans-serif; font-size: 14px; font-weight: 600; color: #1A202C;">
                                                    Allianz Partners Middle East & Africa
                                                </h4>
                                                <div class="d-flex align-items-center flex-wrap" style="gap: 6px;">
                                                    <span id="detail_partner_type_badge"
                                                        class="d-inline-flex align-items-center"
                                                        style="padding: 6px 10px; gap: 6px; border-radius: 999px; border: 1px solid #E2E8F0; background: #FFF;">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                                            viewBox="0 0 14 14" fill="none" style="flex-shrink:0;">
                                                            <g clip-path="url(#clip0_1009_403_badge)">
                                                                <path
                                                                    d="M6.99994 5.8333H7.00577M6.99994 8.16682H7.00577M6.99994 3.49978H7.00577M9.33304 5.8333H9.33887M9.33304 8.16682H9.33887M9.33304 3.49978H9.33887M4.66684 5.8333H4.67267M4.66684 8.16682H4.67267M4.66684 3.49978H4.67267M5.25012 12.8339V11.0837C5.25012 10.929 5.31157 10.7806 5.42095 10.6712C5.53034 10.5618 5.6787 10.5003 5.83339 10.5003H8.16649C8.32118 10.5003 8.46954 10.5618 8.57893 10.6712C8.68831 10.7806 8.74976 10.929 8.74976 11.0837V12.8339M3.50029 1.16626H10.4996C11.1439 1.16626 11.6661 1.68864 11.6661 2.33302V11.6671C11.6661 12.3115 11.1439 12.8339 10.4996 12.8339H3.50029C2.85602 12.8339 2.33374 12.3115 2.33374 11.6671V2.33302C2.33374 1.68864 2.85602 1.16626 3.50029 1.16626Z"
                                                                    stroke="#0B4F8A" stroke-width="2"
                                                                    stroke-linecap="round" />
                                                            </g>
                                                            <defs>
                                                        ...
                                                        <span id="detail_partner_type_text"
                                                            style="color: #0B4F8A; font-family: 'Inter', sans-serif; font-size: 12px; font-weight: 700;">Insurance
                                                            Provider</span>
                                                    </span>
                                                    <span id="detail_partner_code"
                                                        style="color: #64748B; font-family: 'Inter', sans-serif; font-size: 12px; font-weight: 400;">Partner
                                                        ID: <span id="detail_partner_code_value">STA-INS-002</span></span>
                                                </div>
                                                <div>
                                                    <a href="#" onclick="displayEdit(currentPartnerData)"
                                                        class="d-inline-flex align-items-center text-decoration-none"
                                                        style="width: 110px; padding: 6px 10px; gap: 6px; border-radius: 999px; background: rgba(25, 94, 153, 0.18); color: #0B4F8A; font-family: 'Inter', sans-serif; font-size: 12px; font-weight: 700;">
                                                        Edit Partner
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                                            viewBox="0 0 14 14" fill="none" style="flex-shrink:0;">
                                        ...
                                        <div class="d-flex flex-column align-items-start"
                                            style="gap: 10px; width: 260px; max-width: 100%;">
                                            <span id="detail_status_badge" class="d-inline-flex align-items-center"
                                                style="padding: 6px 10px; gap: 8px; border-radius: 999px; border: 1px solid #A7F3D0; background: #D1FAE5;">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                                    viewBox="0 0 14 14" fill="none" style="flex-shrink:0;">
                                                    <g clip-path="url(#clip0_1009_406)">
                                                        <path
                                                            d="M12.7168 5.83345C12.9832 7.14087 12.7934 8.50011 12.1789 9.6845C11.5644 10.8689 10.5625 11.8068 9.3402 12.3419C8.1179 12.877 6.7491 12.9768 5.46208 12.6248C4.17505 12.2728 3.0476 11.4903 2.26773 10.4076C1.48786 9.32496 1.10272 8.00767 1.17654 6.67542C1.25036 5.34318 1.77867 4.0765 2.67337 3.08663C3.56807 2.09676 4.77508 1.44353 6.09311 1.23588C7.41114 1.02823 8.76053 1.2787 9.91624 1.94553M5.2498 6.4165L6.9998 8.1665L12.8331 2.33317"
                                                ...
                                                <span id="detail_status_badge_text"
                                                    style="color:#065F46; font-family:'Inter', sans-serif; font-size:12px; font-weight:700;">Active</span>
                                            </span>

                                            <div class="d-flex align-items-start" style="gap: 10px; width: 100%;">
                                                <div class="d-flex flex-column align-items-start"
                                                    style="padding: 10px; gap: 2px; flex: 1 0 0; border-radius: 10px; border: 1px solid #E2E8F0; background: #F8FAFC;">
                                                    <div
                                                        style="color:#64748B; font-family:'Inter', sans-serif; font-size:11px; font-weight:600;">
                                                        TOTAL POPULATION</div>
                                                    <div id="detail_total_population"
                                                        style="color:#0F172A; font-family:'Inter', sans-serif; font-size:16px; font-weight:800;">
                                                        1</div>
                                                </div>
                                                ...
                                                <div class="d-flex flex-column align-items-start"
                                                    ...
                                                    <div
                                                        style="color:#64748B; font-family:'Inter', sans-serif; font-size:11px; font-weight:600;">
                                                        ACTIVE POPULATION</div>
                                                    <div id="detail_active_population"
                                                        style="color:#0F172A; font-family:'Inter', sans-serif; font-size:16px; font-weight:800;">
                                                        1</div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="d-flex align-items-center gap-3 mb-3">
                                                <div class="rounded-3 d-flex align-items-center justify-content-center"
                                                    ...
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                                        viewBox="0 0 18 18" fill="none" style="flex-shrink:0;">
                                                        <path
                                                            d="M10.4997 1.49951H4.50034C4.10255 1.49951 3.72106 1.65756 3.43978 1.93889C3.15851 2.22022 3.00049 2.60178 3.00049 2.99963V15.0006C3.00049 15.3984 3.15851 15.78 3.43978 16.0613C3.72106 16.3427 4.10255 16.5007 4.50034 16.5007H13.4994C13.8972 16.5007 14.2787 16.3427 14.56 16.0613C14.8413 15.78 14.9993 15.3984 14.9993 15.0006V5.99987M10.4997 1.49951C10.7371 1.49913 10.9723 1.54572 11.1916 1.6366C11.4109 1.72748 11.61 1.86086 11.7776 2.02906L14.4683 4.72027C14.637 4.88791 14.7707 5.08731 14.8618 5.30694C14.953 5.52657 14.9997 5.76208 14.9993 5.99987M10.4997 1.49951V5.24981C10.4997 5.44874 10.5787 5.63952 10.7194 5.78018C10.86 5.92085 11.0508 5.99987 11.2497 5.99987L14.9993 5.99987M7.50004 6.74993H6.00019M11.9996 9.75017H6.00019M11.9996 12.7504H6.00019"
                                                            stroke="#1D4ED8" stroke-width="2" stroke-linecap="round" />
                                                    </svg>
                                                </div>
                                                <div>
                                                    <h5 class="fw-bold mb-0 text-dark" style="font-size: 15px;">Contract
                                                        Info</h5>
                                                    <div class="text-muted" style="font-size: 12px;">Key contract details
                                                        and billing settings</div>
                                                </div>
                                            </div>

                                            <div class="d-flex flex-column gap-2">
                                                <!-- Contract Start -->
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                                    style="background-color: #F8FAFC;">
                                                    <div class="d-flex flex-column align-items-center justify-content-center flex-shrink-0"
                                                        style="width: 32px; height: 32px; border-radius: 10px; border: 1px solid #E5E7EB; background: #FFF;">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                            viewBox="0 0 16 16" fill="none">
                                                            <path
                                                    ...
                                                    <div>
                                                        <div class="text-muted text-uppercase"
                                                            style="font-size: 9px; font-weight: 700;">CONTRACT START</div>
                                                        <div id="detail_contract_start" class="fw-semibold"
                                                            style="font-size: 13px; color: #334155;">2025-10-01</div>
                                                    </div>
                                                </div>
                                                <!-- Contract End -->
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                                    ...
                                                    <div>
                                                        <div class="text-muted text-uppercase"
                                                            style="font-size: 9px; font-weight: 700;">CONTRACT END</div>
                                                        <div id="detail_contract_end" class="fw-semibold"
                                                            style="font-size: 13px; color: #334155;">2029-12-02</div>
                                                    </div>
                                                </div>
                                                <!-- Contract No -->
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                                    ...
                                                    <div class="d-flex flex-column align-items-center justify-content-center flex-shrink-0"
                                                        ...
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                            viewBox="0 0 16 16" fill="none">
                                                            <path
                                                                d="M9.33323 1.33277H4.00044C3.64685 1.33277 3.30774 1.47325 3.05772 1.72332C2.8077 1.97339 2.66724 2.31256 2.66724 2.66621V13.3337C2.66724 13.6874 2.8077 14.0265 3.05772 14.2766C3.30774 14.5267 3.64685 14.6672 4.00044 14.6672H11.9996C12.3532 14.6672 12.6923 14.5267 12.9423 14.2766C13.1924 14.0265 13.3328 13.6874 13.3328 13.3337V5.33309M9.33323 1.33277C9.54425 1.33242 9.75325 1.37384 9.94819 1.45462C10.1431 1.53541 10.3202 1.65396 10.4691 1.80347L12.8609 4.19566C13.0108 4.34468 13.1296 4.52192 13.2106 4.71714C13.2916 4.91237 13.3332 5.12171 13.3328 5.33309M9.33323 1.33277V4.66636C9.33323 4.84319 9.40347 5.01277 9.52848 5.1378C9.65349 5.26284 9.82304 5.33308 9.99983 5.33308L13.3328 5.33309"
                                                                stroke="#0B4F8A" stroke-width="2" stroke-linecap="round" />
                                                        </svg>
                                                    </div>
                                                    <div>
                                                        <div class="text-muted text-uppercase"
                                                            style="font-size: 9px; font-weight: 700;">CONTRACT NO.</div>
                                                        <div id="detail_contract_no" class="fw-semibold"
                                                            style="font-size: 13px; color: #334155;">CONTRACT2025</div>
                                                    </div>
                                                </div>
                                                <!-- Contract File -->
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                                    ...
                                                    <div>
                                                        <div class="text-muted text-uppercase"
                                                            style="font-size: 9px; font-weight: 700;">CONTRACT FILE</div>
                                                        <a id="detail_contract_file" href="#" target="_blank"
                                                            class="fw-bold" style="font-size: 13px; display: none;"></a>
                                                        <div id="detail_contract_file_empty" class="fw-semibold"
                                                            style="font-size: 13px; color: #334155;">-</div>
                                                    </div>
                                                </div>
                                                <!-- Billing Type -->
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                                    ...
                                                    <div class="d-flex flex-column align-items-center justify-content-center flex-shrink-0"
                                                        ...
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                            ...
                                                            <g clip-path="url(#clip0_1009_427)">
                                                                <path
                                                                    d="M11.3333 6.66652L14 3.99964L11.3333 1.33276M14 3.99964H4.66667C3.95942 3.99964 3.28115 4.28062 2.78105 4.78075C2.28095 5.28089 2 5.95922 2 6.66652V7.33324M4.66667 9.3334L2 12.0003L4.66667 14.6672M2 12.0003H11.3333C12.0406 12.0003 12.7189 11.7193 13.219 11.2192C13.719 10.719 14 10.0407 14 9.3334V8.66668"
                                                                    stroke="#0B4F8A" stroke-width="2"
                                                                    stroke-linecap="round" />
                                                            </g>
                                                            <defs>
                                                    ...
                                                    <div>
                                                        <div class="text-muted text-uppercase"
                                                            style="font-size: 9px; font-weight: 700;">BILLING TYPE</div>
                                                        <div id="detail_billing_type" class="fw-semibold"
                                                            style="font-size: 13px; color: #334155;">Per Member</div>
                                                    </div>
                                                </div>
                                                <!-- Default Currency -->
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                    ...
                                    <div class="col-lg-6">
                                        <div class="border rounded-4 p-4 h-100 bg-white"
                                            ...
                                            <div class="d-flex align-items-center gap-3 mb-3">
                                                ...
                                                <div>
                                                    <h5 class="fw-bold mb-0 text-dark" style="font-size: 15px;">Contact Info
                                                    </h5>
                                                    <div class="text-muted" style="font-size: 12px;">Primary contact details
                                                ...
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                                    ...
                                                    <div class="d-flex flex-column align-items-center justify-content-center flex-shrink-0"
                                                        ...
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                            viewBox="0 0 16 16" fill="none">
                                                            <path
                                                                d="M12.6672 14V12.6667C12.6672 11.9594 12.3862 11.2811 11.886 10.781C11.3859 10.281 10.7075 10 10.0002 10H5.99974C5.29241 10 4.61406 10.281 4.1139 10.781C3.61375 11.2811 3.33276 11.9594 3.33276 12.6667V14M10.6669 4.66667C10.6669 6.13943 9.47289 7.33333 7.99996 7.33333C6.52704 7.33333 5.33299 6.13943 5.33299 4.66667C5.33299 3.19391 6.52704 2 7.99996 2C9.47289 2 10.6669 3.19391 10.6669 4.66667Z"
                                                                stroke="#C2410C" stroke-width="2" stroke-linecap="round" />
                                                        </svg>
                                                    </div>
                                                    <div>
                                                        <div class="text-muted text-uppercase"
                                                            style="font-size: 9px; font-weight: 700;">CONTACT PERSON</div>
                                                        <div id="detail_contact_person" class="fw-semibold"
                                                            style="font-size: 13px; color: #334155;">Mr X</div>
                                                    </div>
                                                </div>
                                                <!-- Designation -->
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                                    ...
                                                    <div>
                                                        <div class="text-muted text-uppercase"
                                                            style="font-size: 9px; font-weight: 700;">DESIGNATION</div>
                                                        <div id="detail_contact_designation" class="fw-semibold"
                                                            style="font-size: 13px; color: #334155;">CEO</div>
                                                    </div>
                                                </div>
                                                <!-- Email -->
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                                    ...
                                                    <div class="d-flex flex-column align-items-center justify-content-center flex-shrink-0"
                                                        ...
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                            viewBox="0 0 16 16" fill="none">
                                                            <path
                                                                d="M14.6672 4.66704L8.67268 8.48465C8.46926 8.60278 8.23821 8.66501 8.00296 8.66501C7.76772 8.66501 7.53666 8.60278 7.33324 8.48465L1.33276 4.66704M2.6662 2.66724H13.3337C14.0702 2.66724 14.6672 3.26413 14.6672 4.00044V11.9996C14.6672 12.7359 14.0702 13.3328 13.3337 13.3328H2.6662C1.92977 13.3328 1.33276 12.7359 1.33276 11.9996V4.00044C1.33276 3.26413 1.92977 2.66724 2.6662 2.66724Z"
                                                                stroke="#C2410C" stroke-width="2" stroke-linecap="round" />
                                                        </svg>
                                                    </div>
                                                    <div>
                                                        <div class="text-muted text-uppercase"
                                                            style="font-size: 9px; font-weight: 700;">EMAIL</div>
                                                        <div id="detail_contact_email" class="fw-semibold"
                                                            style="font-size: 13px; color: #334155;">MrX@star.com</div>
                                                    </div>
                                                </div>
                                                <!-- Invoice Email -->
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                                    ...
                                                    <div>
                                                        <div class="text-muted text-uppercase"
                                                            style="font-size: 9px; font-weight: 700;">INVOICE EMAIL</div>
                                                        <div id="detail_invoice_email" class="fw-semibold"
                                                            style="font-size: 13px; color: #334155;">Invoice@star.com</div>
                                                    </div>
                                                </div>
                                                <!-- Phone -->
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                                    ...
                                                    <div class="d-flex flex-column align-items-center justify-content-center flex-shrink-0"
                                                        ...
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                            ...
                                                            <g clip-path="url(#clip0_1009_448)">
                                                                <path
                                                                    d="M9.66122 11.0865C9.51422 11.1232 9.35909 11.1088 9.2214 11.0455C7.36953 10.1367 5.87091 8.63993 4.95972 6.7892C4.89224 6.65077 4.87476 6.49325 4.91024 6.3434C4.94573 6.19354 5.032 6.06059 5.1544 5.96713L5.46643 5.73312C5.63204 5.60891 5.76645 5.44785 5.85903 5.2627C5.95161 5.07754 5.9998 4.87337 5.9998 4.66636V2.6662C5.9998 2.31255 5.85932 1.97339 5.60925 1.72332C5.35918 1.47325 5.02001 1.33276 4.66636 1.33276H2.6662C2.31255 1.33276 1.97339 1.47325 1.72332 1.72332C1.47325 1.97339 1.33276 2.31255 1.33276 2.6662C1.33276 5.84906 2.59715 8.90155 4.84776 11.1522C7.09838 13.4028 10.1509 14.6672 13.3337 14.6672C13.6874 14.6672 14.0265 14.5267 14.2766 14.2766C14.5267 14.0265 14.6672 13.6874 14.6672 13.3337V11.3336C14.6672 10.9799 14.5267 10.6407 14.2766 10.3907C14.0265 10.1406 13.6874 10.0001 13.3337 10.0001H11.3336C11.1266 10.0001 10.9224 10.0483 10.7372 10.1409C10.5521 10.2335 10.391 10.3679 10.2668 10.5335L10.0301 10.8435C9.93834 10.9641 9.80823 11.0498 9.66122 11.0865Z"
                                                    ...
                                                    <div>
                                                        <div class="text-muted text-uppercase"
                                                            style="font-size: 9px; font-weight: 700;">PHONE</div>
                                                        <div id="detail_contact_phone" class="fw-semibold"
                                                            style="font-size: 13px; color: #334155;">0021671225566</div>
                                                    </div>
                                                </div>
                                                <!-- Alt Phone -->
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                                    ...
                                                    <div>
                                                        <div class="text-muted text-uppercase"
                                                            style="font-size: 9px; font-weight: 700;">ALT PHONE</div>
                                                        <div id="detail_alt_phone" class="fw-semibold"
                                                            style="font-size: 13px; color: #334155;">-</div>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div class="row g-4 mt-0">
                                    ...
                                    <div class="col-lg-6">
                                        <div class="border rounded-4 h-100 bg-white d-flex flex-column align-items-start"
                                            style="border-color: #E2E8F0 !important; padding: 20px 24px 24px 24px; gap: 16px;">
                                            <div class="d-flex align-items-center gap-3">
                                                <div class="rounded-3 d-flex align-items-center justify-content-center"
                                                    style="width: 40px; height: 40px; background-color: #F3E8FF; color: #9333EA;">
                                                ...
                                                <div>
                                                    <h5 class="fw-bold mb-0 text-dark" style="font-size: 15px;">Location
                                                        Info</h5>
                                                    <div class="text-muted" style="font-size: 12px;">Primary contact details
                                                        for Location</div>
                                                </div>
                                            ...
                                            <div class="d-flex flex-column gap-2 w-100">
                                                ...
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                                    ...
                                                    <div class="d-flex align-items-center justify-content-center rounded-3 bg-white flex-shrink-0"
                                                        style="width: 32px; height: 32px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                            viewBox="0 0 24 24" fill="none" stroke="#9333EA"
                                                            stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                                            <path
                                                                d="M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0" />
                                                            <circle cx="12" cy="10" r="3" />
                                                    ...
                                                    <div>
                                                        <div class="fw-bold" style="font-size: 12px; color: #475569;">
                                                            Address 1</div>
                                                        <div id="detail_address1"
                                                            style="font-size: 13px; font-weight: 400; color: #64748B;">
                                                            Eiffel 2</div>
                                                    </div>
                                                </div>
                                                <!-- Address 2 -->
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                                    ...
                                                    <div>
                                                        <div class="fw-bold" style="font-size: 12px; color: #475569;">
                                                            Address 2</div>
                                                        <div id="detail_address2"
                                                            style="font-size: 13px; font-weight: 400; color: #64748B;">Le
                                                            kram</div>
                                                    </div>
                                                </div>
                                                <!-- City -->
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                                    ...
                                                    <div>
                                                        <div class="fw-bold" style="font-size: 12px; color: #475569;">City
                                                        </div>
                                                        <div id="detail_city"
                                                            style="font-size: 13px; font-weight: 400; color: #64748B;">
                                                            cheikh zayed</div>
                                                ...
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                                    ...
                                                    <div>
                                                        <div class="fw-bold" style="font-size: 12px; color: #475569;">Postal
                                                            Code</div>
                                                        <div id="detail_postal_code"
                                                            style="font-size: 13px; font-weight: 400; color: #64748B;">2021
                                                        </div>
                                                    </div>
                                                </div>
                                                <!-- Country -->
                                                <div class="bg-light border rounded-3 p-2.5 px-3 d-flex align-items-center gap-3"
                                                    ...
                                                    <div>
                                                        <div class="fw-bold" style="font-size: 12px; color: #475569;">
                                                            Country</div>
                                                        <div id="detail_country"
                                                            style="font-size: 13px; font-weight: 400; color: #64748B;">UAE
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div class="col-lg-6">
                                        <div class="border rounded-4 h-100 bg-white d-flex flex-column align-items-start"
                                            ...
                                            <div class="d-flex align-items-center gap-3">
                                                <div class="rounded-3 d-flex align-items-center justify-content-center"
                                                    style="width: 40px; height: 40px; background-color: #DCFCE7; color: #16A34A;">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                                ...
                                                <div>
                                                    <h5 class="fw-bold mb-0 text-dark" style="font-size: 15px;">Statistics
                                                    </h5>
                                                </div>
                                            </div>
                                            ...
                                            <div class="row g-2 w-100">
                                                ...
                                                <div class="col-6 col-sm-3">
                                                    <div class="stat-mini-card p-3 rounded-3 border d-flex flex-column justify-content-between"
                                                        style="background-color: #F0FDF4; border-color: #DCFCE7 !important; height: 135px;">
                                                        <div class="d-flex justify-content-between align-items-center">
                                                            <div class="d-flex align-items-center justify-content-center rounded-3 bg-white"
                                                                style="width: 28px; height: 28px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
                                                            ...
                                                            <span class="badge rounded-pill bg-success-subtle text-success"
                                                                style="font-size: 10px;">100%</span>
                                                        </div>
                                                        <div>
                                                            <div id="stat_total" class="fw-bold text-dark fs-3 lh-1 mb-1">0</div>
                                                            <div class="text-muted" style="font-size: 11px;">Total</div>
                                                        </div>
                                                        <div id="stat_bar_total" class="stat-mini-bar rounded-pill"
                                                            style="height: 4px; background-color: #10B981;"></div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="col-6 col-sm-3">
                                                    <div class="stat-mini-card p-3 rounded-3 border d-flex flex-column justify-content-between"
                                                        style="background-color: #FFF7ED; border-color: #FFEDD5 !important; height: 135px;">
                                                        <div class="d-flex justify-content-between align-items-center">
                                                            <div class="d-flex align-items-center justify-content-center rounded-3 bg-white"
                                                                style="width: 28px; height: 28px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="14"
                                                            ...
                                                            <span class="badge rounded-pill"
                                                                style="font-size: 10px; color: #EA580C !important; background-color: #FFEDD5 !important;">100%</span>
                                                        </div>
                                                        <div>
                                                            <div id="stat_active" class="fw-bold text-dark fs-3 lh-1 mb-1">0</div>
                                                            <div class="text-muted" style="font-size: 11px;">Active</div>
                                                        </div>
                                                        <div id="stat_bar_active" class="stat-mini-bar rounded-pill"
                                                            style="height: 4px; background-color: #F97316;"></div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="col-6 col-sm-3">
                                                    <div class="stat-mini-card p-3 rounded-3 border d-flex flex-column justify-content-between"
                                                        style="background-color: #F5F3FF; border-color: #DDD6FE !important; height: 135px;">
                                                        <div class="d-flex justify-content-between align-items-center">
                                                            <div class="d-flex align-items-center justify-content-center rounded-3 bg-white"
                                                                style="width: 28px; height: 28px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="14"
                                                            ...
                                                            <span class="badge rounded-pill"
                                                                style="font-size: 10px; color: #7C3AED; background-color: #DDD6FE;">100%</span>
                                                        </div>
                                                        <div>
                                                            <div id="stat_group_policy"
                                                                class="fw-bold text-dark fs-3 lh-1 mb-1">0</div>
                                                            <div class="text-muted" style="font-size: 11px;">Group Policy</div>
                                                        </div>
                                                        <div id="stat_bar_group" class="stat-mini-bar rounded-pill"
                                                            style="height: 4px; background-color: #8B5CF6;"></div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="col-6 col-sm-3">
                                                    <div class="stat-mini-card p-3 rounded-3 border d-flex flex-column justify-content-between"
                                                        style="background-color: #EFF6FF; border-color: #DBEAFE !important; height: 135px;">
                                                        <div class="d-flex justify-content-between align-items-center">
                                                            <div class="d-flex align-items-center justify-content-center rounded-3 bg-white"
                                                                style="width: 28px; height: 28px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="14"
                                                        ...
                                                        <div>
                                                            <div id="stat_total_accounts"
                                                                class="fw-bold text-dark fs-3 lh-1 mb-1">0</div>
                                                            <div class="text-muted" style="font-size: 11px;">Total Accounts</div>
                                                        </div>
                                                        <div id="stat_bar_accounts" class="stat-mini-bar rounded-pill"
                                                            style="height: 4px; background-color: #3B82F6;"></div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    <script>
        ...
        /* ── Animate the Statistics mini-cards ── */
        function animateStatCards(vals) {
            var cards = document.querySelectorAll('.stat-mini-card');
            var bars  = document.querySelectorAll('.stat-mini-bar');
            var ids   = ['stat_total', 'stat_active', 'stat_group_policy', 'stat_total_accounts'];
            var barIds = ['stat_bar_total', 'stat_bar_active', 'stat_bar_group', 'stat_bar_accounts'];
            ...
            // Stagger each card: delay = index * 120ms
            cards.forEach(function(card, i) {
                setTimeout(function() {
                    card.classList.add('animated');
                    card.style.animationDelay = '0s';

                    // Count-up the number inside this card
                    var numEl = document.getElementById(ids[i]);
                    countUp(numEl, vals[i] || 0, 900);

                    // Animate the bar with a short delay after card appears
                    setTimeout(function() {
                        var barEl = document.getElementById(barIds[i]);
                        if (barEl) barEl.classList.add('animated');
                    }, 150);
                }, i * 120);
            });
        }
        ...
        function openPartnerDetails(partner) {
            ...
            if (partner) {
                ...
                // Population stats for the detail banner
                let totalPop = parseInt(partner.total_population) || parseInt(partner.policies) || 1;
                let activePop = parseInt(partner.active_population) || 1;
                let el_tp = document.getElementById('detail_total_population');
                let el_ap = document.getElementById('detail_active_population');
                if (el_tp) el_tp.textContent = totalPop;
                if (el_ap) el_ap.textContent = activePop;
                ...
                let desigEl = document.getElementById('detail_contact_designation');
                if (desigEl) desigEl.innerText = partner.designation || partner.contact_designation || '-';

                let invEl = document.getElementById('detail_invoice_email');
                if (invEl) invEl.innerText = partner.invoice_email || partner.contact_email || '-';

                let altPhEl = document.getElementById('detail_alt_phone');
                if (altPhEl) altPhEl.innerText = partner.alt_phone || '-';

                let billEl = document.getElementById('detail_billing_type');
                if (billEl) billEl.innerText = partner.billing_type || '-';

                let currEl = document.getElementById('detail_currency');
                if (currEl) currEl.innerText = partner.currency || '-';
                ...
                // ── Trigger Statistics Card Animations ──
                var policies   = parseInt(partner.policies)   || totalPop;
                var members    = parseInt(partner.members)    || activePop;
                var statVals   = [totalPop, activePop, policies, members];
                // Small delay so the panel transition completes first
                setTimeout(function() {
                    animateStatCards(statVals);
                }, 80);
            }
        }
    </script>
